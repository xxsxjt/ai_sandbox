...
async function checkServiceStatus() {
    try {
        ...
        if (healthResponse.ok) {
            ...
            // 更新Java状态
            updateStatusIndicator('java-status', true);
            
            // 更新搜索状态
            const searchStatus = healthData.services?.search === 'healthy';
            updateStatusIndicator('search-status', searchStatus);
            
            // 更新Ollama状态（如果配置了）
            const ollamaStatus = healthData.services?.ollama;
            if (ollamaStatus === 'healthy') {
                updateStatusIndicator('ollama-status', true);
            } else if (ollamaStatus === 'unhealthy') {
                updateStatusIndicator('ollama-status', false);
                console.warn('Ollama服务不可用：', ollamaStatus);
            } else {
                // 未配置，显示灰色
                updateStatusIndicator('ollama-status', null);
            }
            
            return healthData;
        } else {
            // Java后端不可用
            updateStatusIndicator('java-status', false);
            updateStatusIndicator('search-status', false);
            updateStatusIndicator('ollama-status', false);
            console.error('Java后端健康检查失败:', healthResponse.status);
        }
    } catch (error) {
...
function updateStatusIndicator(elementId, status) {
    ...
    if (status === true) {
        ...
        element.style.backgroundColor = '#F44336'; // 红色
        element.title = '服务异常';
    } else {
        element.style.backgroundColor = '#9E9E9E'; // 灰色
        element.title = '未配置';
    }
}

// 测试搜索API
async function testSearchAPI(query = '测试') {
    try {
        const response = await fetch(`/api/search?q=${encodeURIComponent(query)}`, {
            timeout: 10000
        });
        ...
        if (response.ok) {
            const data = await response.json();
            return { success: true, results: data.results?.length || 0 };
        } else {
...
// 测试Ollama API
async function testOllamaAPI(endpoint = 'http://localhost:11434/v1') {
    try {
